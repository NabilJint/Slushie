...
Implement the AI Teacher audio lesson screen exactly as shown in the attached design using `DESIGN.md` branding. When the user taps any lesson from the Learn/Lessons screen, open this screen with the selected lesson id and display the selected lesson’s language, title, goal, phrases, and AI teacher context from the hardcoded learning data.
